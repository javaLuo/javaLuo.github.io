...
| chart\_property\_page\_style              | on       |          | 图标属性弹框中大“样式”选项，即右上角齿轮里面的           |
...
| caption\_buttons\_text\_if\_possible      | off      | 1.4      | 标题和按钮尽量图标和文字都显示而不是只显示图标             |
...
| source\_selection\_markers                | on       | 1.11     | 线条上打点标记，禁用里就不能点选做标记                     |
...
| 字段名                     | 默认状态 | 版本 | 描述                                          |
| -------------------------- | -------- | ---- | --------------------------------------------- |
| move\_logo\_to\_main\_pane | off      |      | LOGO放到主窗口中而不是底部窗口,没看出什么效果 |
...
| constraint\_dialogs\_movement                | on       |      | 限制弹出的对话框在图表区域内不能移到外面去,貌似没效果啊                                  |
...
| cl\_feed\_return\_all\_data                  | off      | 1.11 | 当图表获取到的数据多余它需要的时候，也把多余的数据显示出来                               |
